...
extern char SOUNDBANK__0, SOUNDBANK__1, SOUNDBANK__2; // >64K: sampled kit
...
u8 splCd;                      // splash cooldown (vs waterline chatter)
...
// race SFX: slot 3 = the water splash (16kHz -> pitch 4). Fired on
// every air->water re-entry, volume by impact - the sploosh of
// bouncing along. (The continuous engine hum was dropped: SNESMod
// effect voices can't hold a loop and the ARAM went to sampled
// instruments instead.)
#define SFX_SPLASH(vol) spcEffect(4, 3, (u8)((vol) * 16 + 8))
...
static void courseLoad(u8 c)
{
    ...
    if (crsMod[c] != curMod)
    {
        ...
        {
            spcLoad(curMod);
            for (bi = 0; bi < 4; bi++)  // tick/back/boom/splash =
                spcLoadEffect(bi);          // slots 0-3 (call order)
    ...
    while (!raceDone)
    {
                 ...
                 && !(ovlPrev & KEY_START))
        {
            ...
            else
            {
                ...
                // (no engine to quiet - splash is one-shot)
            }
            ovlPrev = pad0;
            while (1)
            {
        ...
        if (inWater)
        {
            ...
            if (!wasInWater)
            {
                ...
                // the SPLOOSH: every real water re-entry, not just big
                // landings - volume by impact speed, a short cooldown so
                // a bob right at the waterline can't machine-gun it
                if (!attract && !split && splCd == 0 && -skiVv >= 48)
                {
                    u8 sv = (u8)((-skiVv) >> 6);
                    SFX_SPLASH(sv > 11 ? 15 : 4 + sv);
                    splCd = 4;
                }
                skiVv >>= 2;
            }
        ...
        if (splCd) // splash cooldown counts down once per tick
            splCd--;
