...
# ---- jet ski sprite ---------------------------------------------------------
# 32x64 rear view, hand-authored: an arcade-proportioned rider (the old
# 32x32 art read as a console racer; doubling the racers' height makes the
# scale intimate). Rows 44-63 are the original console/grips/hull VERBATIM,
# so every waterline/window/physics constant still lines up - only the
# rider above them is new. Chars map to OBJ palette indices below.
...
def ski_frame(shear):
    """32x64 index grid; shear shifts rows above the hull toward +x."""
    grid = [[0] * 32 for _ in range(64)]
    assert len(SKI_ART) == 64 and all(len(r) == 32 for r in SKI_ART)
...
def build_tall_sheet():
    ...
    blit_tall(ski_frame(0.0), 0, 0, 32, 64)    # top 0, bottom 64
    blit_tall(ski_frame(0.14), 32, 0, 32, 64)  # top 4, bottom 68
